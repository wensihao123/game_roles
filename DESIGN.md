...
role = 需要多轮工作/验收回环;skill = 一次性交互或编译。共 **11 role + 6 skill**。

```
┌─ 立项期(每游戏一次)──────────────────────────────────────┐
│  /g-kickoff (skill)    点子 → 目标玩家/核心体验/设计支柱      │
│                         → v1 范围 → GAME-BRIEF.md            │
│                         → 引导四事实源开局 + Producer 初版     │
├─ 功能期(逐 feature 循环)─────────────────────────────────┤
│  /g-design-jam (skill)  功能点子前门 → IDEA.md               │
│  /g-designer  /g-explorer  /g-planner  /g-implementer        │
│  /g-reviewer  /g-art-spec  /g-integrator                     │
├─ 调优期(回路,随时进出)─────────────────────────────────┤
│  /g-playtest (role)    试玩脚本+手感参数方案 ↔ 人玩回报闭环   │
│  /g-bugfix (role)      轻量 bug 通道:复现→根因→修→回归       │
├─ 发布期(每版本一次)──────────────────────────────────────┤
│  /g-release (role)     导出/回归/版本号/上架 ↔ 人执行回报闭环 │
└─ 横跨全程 ────────────────────────────────────────────────┘
   /g-producer (常驻 role)          范围守门 + INBOX 分诊 + 归档
   /g-arch-guard /g-num-smith       四事实源守护 (skill)
...
Game_Roles/
  ...
  roles/
    ...
    feature/   designer.md explorer.md planner.md implementer.md
               reviewer.md art-spec.md integrator.md
    tuning/    playtest.md bugfix.md
    release/   release.md
  skills/      g-kickoff/ g-design-jam/
...
| 单元 | 内嵌蒸馏物 | 理论来源(✅完工 ⬜待补) |
|---|---|---|
| g-kickoff | 核心体验框架(Fantasy/Emotional Goal/Core Loop/支柱);目标玩家画像问题组;默认价值优先序(正确安全→可理解→可控→可恢复→核心体验→深度→效率→新鲜感→装饰);原则冲突记录格式;证据等级表 | philosophy 三篇 ✅ |
| g-designer | "如何评审一个设计"七问(Goal/体验/规则/反馈/成本/取舍/验证)作 FEATURE-DESIGN 自检;每份设计必答"支持哪些支柱/牺牲哪些/是否值得" | design-principles ✅;clarity-and-feedback ⬜ |
| g-planner | P1~P6 决策路由表(问题类型→查哪篇);模块边界判据;合格任务七条标准;DoR 就绪门 | patterns、module-boundaries、task-breakdown、definition-of-ready ✅ |
...
| g-playtest | 十二维测试清单→试玩脚本;假设驱动试玩(验证什么/失败标准/防自欺) | testing-checklist ✅;iteration-and-validation ⬜ |
| g-bugfix | 九步 bug 流(复现→严重度→影响面→根因→修→回归→相邻系统→补测试?→更新文档?) | bugfix-workflow ✅ |
| g-release | 十步发布流(冻结→回归→存档兼容→导出测试→版本号→changelog→构建→发布→验证→回滚) | release-workflow ✅ |
| g-art-spec / g-integrator | AI 资产纪律(风格/角色一致性、透明背景、贴纸白边、spritesheet/pivot、导入参数) | ai-asset-workflow、asset-workflow ✅ |
| g-state-machine | 十类 FSM 分类坐标系(判断"这属于哪类机",按类下钻) | fsm-index ✅ |
| g-arch-guard | SOLID-in-Godot 判据摘要 | solid-in-godot ✅ |
...
- **下钻只在三种情形**(写死在 spec,防滥用烧上下文):
  ...
  2. **记录原则冲突时**——按 philosophy 冲突记录格式(冲突/情境/选择/理由/后果/复审条件)写进 artifact 之前,读原文;
  3. **人明确要求时**。
  日常工作只用内嵌蒸馏层,零额外读取。
- **理论待补标记**:下钻引用指向未完工文档时写 `(理论待补:design/philosophy/xxx.md)`;这些标记汇总即理论库的待办清单。
...
- **四事实源全对称**:planner 必读四份、冲突各自路由(补 UX-MAP,修 gate 不对称);explorer 四份列为可选输入;implementer 的 escalation 补全 `/g-arch-guard` `/g-num-smith` `/g-state-machine` `/g-ux-design` 具体路由(修 Auto 模式引用落空);四 skill 输入对称(arch-guard 补读 FEATURE-DESIGN)。
...
- wikilink 等格式约定四 skill 统一(实施时定一种,全体遵守)。
...
### 6.1 /g-kickoff(skill,立项仪式,每游戏一次)
...
- **工作流**(对话式,一次一问):
  ...
  5. v1 范围切割:垂直切片式"可上架最小版本"+ 明确不做什么;
  6. 风险清单:最可能杀死项目的 2~3 个假设,各配最便宜的验证方式。
- **产出**:`harness/GAME-BRIEF.md`(六项)+ 代填第 0 节的 project-context.md 草稿。
- **收尾**:打印后续仪式清单——四事实源开局(各跑一次模式 A,读 GAME-BRIEF 建初版)→ `/g-producer` 建初版 BACKLOG。立项期完成判据:GAME-BRIEF + project-context + 四事实源 + BACKLOG 五件套齐活。

### 6.2 /g-playtest(role,规格+验收型)
...
- **两个升级出口**:根因牵扯架构/数值/状态机 → 转对应事实源 skill;修复超出"最小修复"(多文件/改接口)→ 升级为正式 feature,Producer 记账。
- **护栏**:修复必须先有失败复现、后有通过验证,均记录在 BUG.md。

### 6.4 /g-release(role,规格+验收型,每版本一次)
...
2. **功能期 role**(从旧体系蒸馏移植 + 修缮 + 理论内嵌):designer / explorer / planner / implementer / reviewer / art-spec / integrator;
3. **skills**:g-design-jam 移植修缮;四事实源 skill 移植 + 5.4 修缮;g-kickoff 新写;
4. **新环节 role**:playtest / bugfix / release;producer 移植修缮;
5. **命令接线**:与旧体系同款安装方式——每个 role/skill 一个 `~/.claude/skills/g-<name>/SKILL.md`(本仓库为源,安装为副本,改源后重拷 + `diff -q` 核对);端到端验证。
...
## 8. 理论待补清单(回流给 GameDesignDocs)

实施后,以下文档完工时回来重蒸馏对应 spec:

- `design/philosophy/clarity-and-feedback.md` → g-designer 的反馈意图细则;
- `design/philosophy/iteration-and-validation.md` → g-playtest 的验证方法论;
- (后续新完工的 philosophy 篇目,按各 role 下钻标记逐个对号。)
